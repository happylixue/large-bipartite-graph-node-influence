...
def sort_bipartite(in_file_name, left_pivot_out_file_name, right_pivot_out_file_name,bucket_size_expect=100):
    ...
    with open(left_pivot_out_file_name, 'wb') as f:
        for idx in range(0, n_buckets):
            list_to_sort = []
            byteblock = bucket_fh_left_pivot[idx].read(12)
            while len(byteblock) == 12:
                left_node_id, right_node_id, left_node_count = struct.unpack('i', byteblock[0:4])[0], \
                                                               struct.unpack('i', byteblock[4:8])[
                                                                   0], struct.unpack('i', byteblock[8:12])[0]
                list_to_sort.append((left_node_id, right_node_id, left_node_count))
                byteblock = bucket_fh_left_pivot[idx].read(12)
            list_to_sort = list(set(list_to_sort))  # de-duplicate
            list_to_sort.sort(key=itemgetter(0, 1, 0))
    ...
    with open(right_pivot_out_file_name, 'wb') as f:
        for idx in range(0, n_buckets):
            list_to_sort = []
            byteblock = bucket_fh_right_pivot[idx].read(12)
            while len(byteblock) == 12:
                left_node_id, right_node_id, left_node_count = struct.unpack('i', byteblock[0:4])[0], \
                                                               struct.unpack('i', byteblock[4:8])[0], \
                                                               struct.unpack('i', byteblock[8:12])[0]
            ...
            list_to_sort.sort(key=itemgetter(1, 0, 0))
            for edge in list_to_sort:
                f.write(struct.pack('i', edge[0]))
                f.write(struct.pack('i', edge[1]))
                f.write(struct.pack('i', edge[2]))
...
if __name__ == "__main__":
    ...
    t_start = time.clock()
    sort_bipartite("data\graph_bipartite_L1.bin", "data\graph_bipartite_L_temp.bin", "data\graph_bipartite_R1.bin")
    t_stop = time.clock()
    print("")
